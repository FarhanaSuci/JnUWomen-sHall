...
<head>
  <meta charset="utf-8">
  <title>Applications for Seat Cancellation</title>
...
<h2 style="text-align:center">Applications for Seat Cancellation</h2><hr>
 ...
 <div class="col-md-3">
  ...
  <div class="divmain">
   <?php
      ...
      $floorNo= $_SESSION['assigned_floor_no'];
        $sql="SELECT * from seatcancellation_applications where seatcancellation_applications.floorNo= $floorNo
        AND approval_status='Waiting' order by serial desc ";
        $query=mysqli_query($conn,$sql);
       while($info=mysqli_fetch_array($query)){  ?>
        <div class="card" style="margin-bottom:3%;">
        <div class="card-body">
         <h5 class="card-title"><strong> Serial: </strong><?php echo $info['serial'];?><br>
         <?php echo  date('d-F-Y',strtotime($info['date'])); ?></h5><hr>
         <p class="card-text"><strong>Name: </strong><?php echo $info['name']; ?><br>
         <strong>Id: </strong><?php echo $info['id'];?><br>
       <?php  $info['floorNo']; $fn=$info['floorNo'];?>
         ...
         <strong>Attached File: </strong><a target="_blank" href="file/<?php echo $info['file']; ?>"><?php echo $info['file']; ?></a><br>
          ...
          <form class= "" action="applications_for_seat_cancellations_HouseTutor.php" method="post">
          ...
          <input type="text" name="sn" value="<?php echo $info['seatNo']; ?>" hidden>
          <input type="text" name="rn" value="<?php echo $info['roomNo']; ?>" hidden>  
  ...
  if(isset($_POST['submit']))
   {     
    ...
    $rn=$_POST['rn'];
    $sql="UPDATE students 
    Set previous_seatNo='$sn', previous_roomNo='$rn',previous_floorNo='$fn', floorNo='0', roomNo='0', seatNo='canceled'
     where students.id= '$sid' ";
    $query=mysqli_query($conn,$sql);
    $sql="UPDATE seatcancellation_applications 
    Set approval_status='Approved' where serial= '$serial'' ";
    $query=mysqli_query($conn,$sql);
    echo '<script type="text/JavaScript">
    alert("This application has been accepted and the seat has been canceled!");
    window.location.href="applications_for_seat_cancellations_HouseTutor.php";
    </script>'
    ;
   }
 ...
 <div class="col-md-3">
  ...
  <div class="divmain">
   <?php
      include 'db_conn.php';
        $sql="SELECT * from seatcancellation_applications where seatcancellation_applications.floorNo= $floorNo
        AND approval_status='Approved' order by serial desc ";
        $query=mysqli_query($conn,$sql);
       while($info=mysqli_fetch_array($query)){  ?>
        <div class="card" style="margin-bottom:3%;">
        <div class="card-body">
 ...
 <div class="col-md-3">
  ...
  <div class="divmain">
   <?php
      include 'db_conn.php';
        $sql="SELECT * from seatcancellation_applications where seatcancellation_applications.floorNo= $floorNo
        AND approval_status='Canceled' order by serial desc ";
        $query=mysqli_query($conn,$sql);
       while($info=mysqli_fetch_array($query)){  ?>
        <div class="card" style="margin-bottom:3%;">
        <div class="card-body">
